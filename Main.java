...
public class Main {
    public static void main(String[] args) {
        ...
        while (opcion != 7) {
            ...
            if (opcion == 7) {
                System.out.println("|||||||||||||||||||||||||||||||||||||||");
                System.out.println("Gracias por usar Chistian959, hasta pronto.");
                System.out.println("|||||||||||||||||||||||||||||||||||||||");
                break;
            }
    ...
    public static void procesarConversion(String simboloInicial, String simboloFinal, String direccion, double cantConversion) {
        ...
        Map<String, Double> conversiones = moneda.getConversiones();

        if (conversiones == null) {
            System.out.println("Error: No se pudieron obtener las conversiones de moneda.");
            return;
        }

        if (!conversiones.containsKey(simboloFinal)) {
            System.out.println("Error: No se encontró la tasa de cambio para la moneda " + simboloFinal);
            return;
        }

        double tasaConversion = conversiones.get(simboloFinal);

        DecimalFormat df = new DecimalFormat("#,###.00");
        double resultado = moneda.calcularConversion(cantConversion, tasaConversion);
